...
def get_path_from_json(camera_path: Dict[str, Any]) -> Cameras:
    ...
    for camera in camera_path["camera_path"]:
        # pose
        c2w = camera["camera_to_world"].view(4, 4)[:3]
        c2ws.append(c2w)
        if (
            camera_type == CameraType.EQUIRECTANGULAR
...
def get_camera(transform,height: int = 300, width: int = 300, fov_:float= 90) -> Cameras:
    """Takes a transform and returns a trajectory as a Camera instance.
    ...
    # pose
    # c2w = torch.tensor(transform,requires_grad=True)[:3]
