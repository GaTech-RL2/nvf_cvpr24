...
class RaySamples(TensorDataclass):
    ...
    def get_opacity(self, densities: Float[Tensor, "*batch num_samples 1"]) -> Float[Tensor, "*batch num_samples 1"]:
        """Return opacity based on predicted densities

        Args:
            densities: Predicted densities for samples along ray

        Returns:
            opacity for each sample
        """
        delta_density = self.deltas * densities
