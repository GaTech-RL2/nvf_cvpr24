...
class RGBVarianceRenderer(nn.Module):
    """Calculate rgb uncertainty along the ray."""

    @classmethod
    def forward(
        cls, betas: Float[Tensor, "*bs num_samples 1"], 
        weights: Float[Tensor, "*bs num_samples 1"],
        ray_indices: Optional[Int[Tensor, "num_samples"]] = None,
        num_rays: Optional[int] = None,
    ) -> Float[Tensor, "*bs 3"]:
        ...
        Returns:
            Rendering of rgb uncertainty.
        """
        if ray_indices is not None and num_rays is not None:
            rgb_variance = nerfacc.accumulate_along_rays(weights[..., 0], values=betas, ray_indices=ray_indices, n_rays=num_rays)
        else:
            rgb_variance = torch.sum(weights * betas, dim=-2)
        return rgb_variance
...
class WeightDistributionUncertaintyRenderer(nn.Module):
    """Calculate ray-based volumentric uncertainty based on weight distribution along the ray."""
    def __init__(self, background_color: BackgroundColor = "random") -> None:
        super().__init__()
        self.background_color: BackgroundColor = background_color
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            ...
            entropy.index_add_(0, ray_indices, weights * torch.nan_to_num(torch.log(weights)) * torch.nan_to_num(ray_samples.deltas))
            entropy = -entropy
        else:            
            entropy = -torch.sum(weights * torch.nan_to_num(torch.log(weights)) * ray_samples.deltas, dim=1)
