...
class InstantNGP(DataParser):
    ...
    # NEED TO TEST
    def update_images(self, img_list, transform_list):
        json_path = ""
        if self.config.data.suffix == ".json":
            meta = load_from_json(self.config.data)
            json_path = self.config.data
            data_dir = self.config.data.parent
        else:
            meta = load_from_json( (self.config.data / "transforms.json").absolute())
            json_path = self.config.data / "transforms.json"
            data_dir = self.config.data


        idx_start = len(os.listdir(str(data_dir) + "/img/"))

        for i in range(idx_start, idx_start + len(img_list)):
            img = img_list[i-idx_start]
            img_name = f"{i:04}.png"
            transform = transform_list[i-idx_start]
            meta["frames"].append({"file_path": "img/"+img_name, "transform_matrix": transform.tolist()})
            # img_path = img_path_list[i]
            # shutil.copy(img_path, data_dir / "img")
            pil_img = Image.fromarray(img, 'RGBA')
            image_path = str(data_dir) + "/img/"+img_name
