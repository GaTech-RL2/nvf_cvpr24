...
class NeurARField(Field):
    ...
    Args:
        ...
        use_rgb_uncertainty: wether to estimate uncertainty in rgb values
    ...
    def get_outputs(
        ...
        # rgb_uncertainty
        if self.use_rgb_variance:
            x = self.mlp_rgb_variance(h).view(*outputs_shape, -1).to(directions)
            x = torch.exp(x)
            outputs[FieldHeadNames.RGB_VARIANCE] = self.field_head_rgb_variance(x)
