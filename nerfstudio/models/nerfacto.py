...
class NerfactoModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.config.use_neur_ar or self.config.use_active_nerf:
            rgb_variance = self.renderer_rgb_variance(betas=field_outputs[FieldHeadNames.RGB_VARIANCE], weights=weights)
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.config.use_neur_ar:
            rgb_variance_gt = (image[:,0:3]-pred_rgb)**2
            loss_dict["neur_ar_loss"] = 1e-6*self.rgb_variance_loss(outputs["rgb_variance"],rgb_variance_gt)

        if self.config.use_active_nerf:
            rgb_variance_gt = (image[:,0:3]-pred_rgb)**2
            self.regularization_coeff = 1e-3
            # add regularization loss to ensure sparser volume density
            loss_dict["active_nerf_loss"] = 1e-5*(torch.mean((1 / (2*(rgb_variance_gt.mean()+1e-9).unsqueeze(-1))) * rgb_variance_gt) + 0.5*torch.mean(torch.log(rgb_variance_gt.mean()+1e-9)) + self.regularization_coeff * outputs["accumulation"].mean() + 4.0)
